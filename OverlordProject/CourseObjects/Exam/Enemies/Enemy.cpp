...
void Enemy::Update(const GameContext& gameContext)
{
	...
	if (FastDistance(m_pEnemy->GetTransform()->GetPosition(), m_LeftPoint) < 4.0f)
	{
		m_Sign = -1;
	}
	if (FastDistance(m_pEnemy->GetTransform()->GetPosition(), m_RightPoint) < 4.0f)
	{
		m_Sign = 1;
	}
	...
	m_Pos = { m_Pos.x + posss.x*gameContext.pGameTime->GetElapsed()*m_Speed * m_Sign
		,m_Pos.y + posss.y*gameContext.pGameTime->GetElapsed()*m_Speed * m_Sign,
		m_Pos.z + posss.z*gameContext.pGameTime->GetElapsed()*m_Speed * m_Sign };
	m_pEnemy->GetTransform()->Translate(m_Pos.x, m_Pos.y, m_Pos.z);
...
const bool Enemy::IsKilled() const
{
	return m_IsTouched;
}
...
void Enemy::SetDamageToPlayer()
{
	m_DamagedPlayer = true;
}
bool Enemy::DidDamage()
{
	if (m_DamagedPlayer && !m_IsTouched)
	{
...
float Enemy::FastDistance(const XMFLOAT3 point1, const XMFLOAT3 point2) const
{

	//no sqrt for because it's performance intensive

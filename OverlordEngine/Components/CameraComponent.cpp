...
XMFLOAT2 CameraComponent::WorldToScreenPoint(const XMFLOAT3& point) const
{
	XMMATRIX vp = XMLoadFloat4x4(&m_ViewProjection);
	XMVECTOR xmPoint = XMLoadFloat3(&point);
	XMVECTOR point3D = XMVector3Transform(xmPoint, vp);
	XMFLOAT3 sp;
	XMStoreFloat3(&sp, point3D);
	float winX = (float)round(((sp.x + 1) / 2.0) * OverlordGame::GetGameSettings().Window.Width);
	float winY = (float)round(((1 - sp.y) / 2.0) * OverlordGame::GetGameSettings().Window.Height);
...
GameObject* CameraComponent::Pick(const GameContext& gameContext, CollisionGroupFlag ignoreGroups) const
{
	...
	int width = windowSettings.Width / 2;
	int height = windowSettings.Height / 2;
	...
	//*********************************************
	//THIS EXISTS TO GET THE X Y Z COMPONENTS!!!!
	//*********************************************

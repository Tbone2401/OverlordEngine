...
#include "../Graphics/ShadowMapMaterial.h"
...
void ModelComponent::Initialize(const GameContext& gameContext)
{
	...
	//Create vertexbuffer for shadow
	if (m_CastShadows)
	{
		ShadowMapMaterial* smm = new ShadowMapMaterial();
		smm->Initialize(gameContext);
		m_pMeshFilter->BuildVertexBuffer(gameContext, smm);
		delete smm;
	}
...
void ModelComponent::SetDraw(bool enabled)
{
	m_Draw = enabled;
}

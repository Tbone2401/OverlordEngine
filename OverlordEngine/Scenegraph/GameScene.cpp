...
void GameScene::RootDraw()
{
	...
	if (m_PostProcessingMaterialIds.size() != 0)
	{
		...
		for (const UINT id : m_PostProcessingMaterialIds)
		{
			//4.1 Get the corresponding PostProcessingMaterial (= CURR_MAT) through MaterialManager
			PostProcessingMaterial* CURR_MAT = m_GameContext.pMaterialManager->GetMaterial_PP(id);
...
void GameScene::AddPostProcessingMaterial(UINT materialId_PP)
{
	...
	if (find(m_PostProcessingMaterialIds.begin(), m_PostProcessingMaterialIds.end(), materialId_PP) == m_PostProcessingMaterialIds.end())
...
void GameScene::RemovePostProcessingMaterial(UINT materialId_PP)
{
	...
	if(!m_PostProcessingMaterialIds.empty())
		m_PostProcessingMaterialIds.erase(find(m_PostProcessingMaterialIds.begin(), m_PostProcessingMaterialIds.end(), materialId_PP));
}

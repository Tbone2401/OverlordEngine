...
bool PhysxProxy::Sweep(const PxGeometry& geometry, const PxTransform& pose, const PxVec3& unitDir, const PxReal distance, PxSweepCallback& hitCall, PxHitFlags hitFlags, const PxQueryFilterData& filterData, PxQueryFilterCallback* filterCall, const PxQueryCache* cache, const PxReal inflation)
{
	if(m_pPhysxScene != nullptr)
	{
		return m_pPhysxScene->sweep(geometry, pose, unitDir, distance, hitCall, hitFlags, filterData, filterCall, cache, inflation);
	}
	Logger::LogError(L"[PhysicsProxy] Sweep failed. Physics scene is a null pointer");
	return false;
}
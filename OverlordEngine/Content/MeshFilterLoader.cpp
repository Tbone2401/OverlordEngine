...
MeshFilter* MeshFilterLoader::LoadContent(const wstring& assetFile)
{
	...
	for (;;)
	{
		...
		case MeshDataType::BLENDINDICES:
		{
			...
			{
				XMFLOAT4 blendIndex = binReader->Read<XMFLOAT4>();
		...
		case MeshDataType::BLENDWEIGHTS:
		{
			...
			for (size_t i = 0; i < vertexCount; i++)
			{
				XMFLOAT4 blendWeight = binReader->Read<XMFLOAT4>();
		...
		case MeshDataType::ANIMATIONCLIPS:
		{
			...
			for (size_t i = 0; i < clipCount; i++)
			{
				...
				//8. For every key
				for (unsigned short k = 0; k < keyCount; k++)
				{
					...
					unsigned short boneTransformCount = binReader->Read<unsigned short>();
					//12. For every transform
					for (size_t b = 0; b < boneTransformCount; b++)
					{
